...
import If from '../../common/layout-operators/If';

class ListItens extends Component{
    //funcao do redux-form: necessario ler a doc para melhor entendimento
    add(index, item = {}) {
        if(!this.props.readOnly) {
            //field = 'credito' = lista de creditos que vem do banco e o formulário manipula
            this.props.arrayInsert('cicloDePagamentoForm', this.props.field, index, item)
        }
    }
    remove(index) {
        if(!this.props.readOnly && this.props.list.length > 1) {
            this.props.arrayRemove('cicloDePagamentoForm', this.props.field, index)
        }
    }
    renderRows() {
        ...
        return list.map((item, index) => (
            <tr key ={index}>
                <td>
                     {/*nomeclatura dinamica a partir do nome do obj que esta no banco*/}
